...
class Seq2Seq(object):
    ...
    #    prints statistics
    def train(self, train_set, valid_set, isSessionRestored):
...
#
        with self.session as sess, self.graph.as_default():#, tf.device("/gpu:0"):
            #with  tf.device("/gpu:0"):
            sess = self.session
            saver = tf.train.Saver()
            if not isSessionRestored:
    ...
    def predict(self, X):
        ...
        dec_op_v = self.session.run(self.decode_outputs_test, feed_dict)
        # dec_op_v is a list; also need to transpose 0,1 indices
        #  (interchange batch_size and timesteps dimensions
        dec_op_v = np.array(dec_op_v).transpose([1, 0, 2])
